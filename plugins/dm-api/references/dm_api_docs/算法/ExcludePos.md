...
**签名:** `string ExcludePos(all_pos,type,x1,y1,x2,y2)`

**描述:** 根据部分Ex接口的返回值，排除指定范围区域内的坐标.
...
| type | int | 取值为0或者1 如果all_pos的内容是由FindPicEx,FindPicMemEx,FindStrEx,FindStrFastEx,FindStrWithFontEx返回，那么取值为0 如果all_pos的内容是由FindColorEx, FindMultiColorEx,FindColorBlockEx,FindShapeEx返回，那么取值为1 如果all_pos的内容是由OcrEx返回，那么取值为2 如果all_pos的内容是由FindPicExS,FindStrExS,FindStrFastExS返回，那么取值为3 |
| x1 | int | 左上角横坐标 |
| y1 | int | 左上角纵坐标 |
| x2 | int | 右下角横坐标 |
| y2 | int | 右下角纵坐标 |
