...
**签名:** `string FindFloat(hwnd, addr_range, float_value_min, float_value_max)`

**描述:** 搜索指定的单精度浮点数, 默认步长是1.默认开启多线程,默认略过Mapped的内存类型,默认是搜索可读可写可执行的内存.如果要定制搜索,请用FindFloatEx
