...
**签名:** `string FindFloatEx(hwnd, addr_range, float_value_min, float_value_max,step,multi_thread,mode)`
...
| float_value_min | float | 搜索的单精度数值最小值 |
| float_value_max | float | 搜索的单精度数值最大值 最终搜索的数值大与等于float_value_min,并且小于等于float_value_max |
| step | int | 搜索步长. |
...
```vbs
// 全局搜索
result = dm.FindFloatEx(hwnd,"00000000-FFFFFFFF",0.5,1.0,4,1,0)
