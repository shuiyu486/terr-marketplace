...
**签名:** `string FindMultiColorEx(x1, y1, x2, y2,first_color,offset_color,sim, dir)`
...
- 返回所有颜色信息的坐标值,然后通过GetResultCount等接口来解析(由于内存限制,返回的坐标数量最多为1800个左右) 坐标是first\_color所在的坐标

## 示例

```vbs
dm_ret = dm.FindMultiColorEx(0,0,2000,2000,
