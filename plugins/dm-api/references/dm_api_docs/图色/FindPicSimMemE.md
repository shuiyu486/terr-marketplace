...
**签名:** `string FindPicSimMemE(x1, y1, x2, y2, pic_info, delta_color,sim,dir)`
...
pos = dm.FindPicMemE(0,0,2000,2000, pic_info,"000000",80,0)
pos = split(pos,"|")
If int(pos(1)) > 0 Then
MessageBox "找到"
End If
...
此接口和FindPicMemE类似. 只不过FindPicSimMemE是以颜色百分比来进行匹配. 如果待查找区域内有杂色,只要颜色百分比达到要求,也一样可以匹配.

这个接口是FindPicMemE的进阶版本. 当sim为100时,那么FindPicSimMemE就退化为FindPicMemE
