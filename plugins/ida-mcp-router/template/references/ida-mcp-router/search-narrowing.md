...
- 已知 `start/end` 时，优先用 `disasm` 分页，而不是 `search_text(start=...)`。
- 追踪 `ctx`、栈局部或字段偏移时，先看入口附近 `rdi` 保存到哪个寄存器/局部，再基于等价基址查写入。
- 判断 `[ctx+offset]` 写入时，不裸搜 offset；只搜索已确认的等价表达式，或分段读取汇编人工筛选。
- call 列表单独查，先限 `count <= 80`，返回地址、目标和角色聚类，不返回完整列表。
...
- `search_text` 未命中只说明 rendered listing 文本未匹配，不等于不存在对应语义；关键 offset/字段必须用寄存器来源、xref、局部 disasm 或数据流再验证。
