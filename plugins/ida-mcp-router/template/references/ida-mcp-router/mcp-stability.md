...
- 是否是多范围、多候选或宽搜索？是则先交给子 agent，不在主会话连发 MCP。
- 是否已有重型 IDA MCP 调用 Running？有则先等待/停止/降级，不追加新查询。
- 是否明确 database/session？不明确时先 health 或 lookup 验证。
- 是否能用 `lookup_funcs` / 小页 `disasm` 替代 `search_text` / 大范围 `insn_query`？
- 是否需要分页？需要时记录 offset/cursor，不把第一页当完整结论。
